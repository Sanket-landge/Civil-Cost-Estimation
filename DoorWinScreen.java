...
public class DoorWinScreen extends JFrame implements ActionListener
    {
        JLabel l1,l2,l3,l4,l5,l6,l7,l8,ll,lb;
        JLabel l9,l10,l11,l12,l13,l14,l15,l16,l17;
        JTextField tcno,tname,tno,tlen,tbred,tdepth,tquan,trate,t1,t2;
	...
	Dimension screen 	= 	Toolkit.getDefaultToolkit().getScreenSize();
		...
		setLocation((screen.width - 620)/2,((screen.height-638)/2)-17);	
                ...
                l8 = new JLabel("Quantity (in m2) :-");
                l9 = new JLabel("Rate (Rs per m2) :-");
                l10 = new JLabel("Total No. :-");
                l11 = new JLabel("Total Length (in m) :-");
                l12 = new JLabel("Total Breadth (in m) :-");
                l13 = new JLabel("Total Depth (in m) :-");
                l14 = new JLabel("Total Quantity (in m2) :-");
                l15 = new JLabel("Rate (Rs per m2) :-");
                ...
                add(ch);
                ch.addItem("Steel Window");
                ch.addItem("Iron Window");
                ch.addItem("Wooden Window");
...
try
{
rs=stm.executeQuery("select * from DoorWin where Custno=(select max(Custno) from DoorWin)");
...
{}
		...
		tno.setEnabled(true);
			...
			if(i==1)
			{
                        ...
                        try
                            {
                                sql = "Insert into DoorWin values('" +tname.getText()+ "','"+ch.getSelectedItem().toString()+"','" +tcno.getText()+"',"+ttno.getText()+"," +ttlen.getText()+"," +ttbred.getText()+"," +ttdepth.getText()+"," +ttquan.getText()+","+ttamt.getText()+")";
                                prstm = cn.prepareStatement(sql);
                                prstm.execute() ;
                                 prstm.close();
			String	sql1 = "Update final set DAmt="+ttamt.getText()+" where Name='"+tname.getText()+"'";
		...
		JOptionPane.showMessageDialog(null,"Calculations Completed Successfully","Calculation",JOptionPane.INFORMATION_MESSAGE);
                        ...
                        try
                            {
                                stm = cn.createStatement();
				             //sql = "Insert into DoorWin values('" +tname.getText()+ "','"+ch.getSelectedItem().toString()+"','" +tcno.getText()+"',"+ttno.getText()+"," +ttlen.getText()+"," +ttbred.getText()+"," +ttdepth.getText()+"," +ttquan.getText()+","+ttamt.getText()+")";
                               stm.executeUpdate ("update DoorWin set Custname='" +tname.getText()+ "',Itemwork='"+ch.getSelectedItem().toString()+"' ,No=" +ttno.getText()+" ,Lenght=" +ttlen.getText()+" ,Breadth=" +ttbred.getText()+" ,Depth=" +ttdepth.getText()+",Quantity=" +ttquan.getText()+",Totamount="+ttamt.getText()+ " where Custno='"+tcno.getText()+"'");//where Custno='"+tcno.getText()+"'");
                               stm.close();
