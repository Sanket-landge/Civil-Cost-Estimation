...
public class BaseConScreen extends JFrame implements ActionListener
    {
        JLabel l1,l2,l3,l4,l5,l6,l7,l8,ll,lb,ld;
        JLabel l9,l10,l11,l12,l13,l14,l15,l16,l17;
        JTextField tname,tcno,tno,tlen,tbred,tdepth,tquan,trate,t1,t2,t3;
	...
	Dimension screen 	= 	Toolkit.getDefaultToolkit().getScreenSize();
        ...
        BaseConScreen()
            {
                super("Base Construction Information......");
                setLayout(null);
                setSize(620,638);
                setVisible(true);
		setLocation((screen.width - 620)/2,((screen.height-638)/2)-17);	
                ...
                l1 = new JLabel("Base Construction");
                ...
                l1.setBounds(140,5,500,40);
                l2.setBounds(5,75,120,40);
                ...
                add(ld);
                ld.setBounds(275,345,30,20);
                t3.setBounds(290,345,30,20);
...
bnew.setForeground(new Color(41,41,221));
		...
		ttbred.setEnabled(false);
		ttdepth.setEnabled(false);
		ttquan.setEnabled(false);
		ttrate.setEnabled(false);
		...
		t3.setEnabled(false);
	        tquan.setEnabled(false);
                ...
                ttno.setEnabled(false);
                ttlen.setEnabled(false);                
                ch.setEnabled(false);
...
try
{
rs=stm.executeQuery("select * from Construction where Custno=(select max(Custno) from Construction)");
...
{}
		...
		ttbred.setEnabled(true);
		ttdepth.setEnabled(true);
		ttquan.setEnabled(true);
		ttrate.setEnabled(true);
		...
		t3.setEnabled(true);
	        tquan.setEnabled(true);
                ...
                ttno.setEnabled(true);
                ttlen.setEnabled(true);         
			...
			if(i==1)
			{
                        try
                            {
                                sql = "Insert into Construction values('" +tname.getText()+ "','"+tcno.getText()+ "','"+ch.getSelectedItem().toString()+"'," +ttno.getText()+"," +ttlen.getText()+"," +ttbred.getText()+"," +ttdepth.getText()+"," +ttquan.getText()+","+ttamt.getText()+ ")";
                                prstm = cn.prepareStatement(sql);
                                prstm.execute() ;
                                
                                prstm.close();

				String	sql1 = "Update final set BAmt="+ttamt.getText()+" where Name='"+tname.getText()+"'";
			...
			if(i==1)
			{
		                ...
		                ttdepth.setText(String.valueOf(Integer.parseInt(tdepth.getText())*Integer.parseInt(t3.getText())));                        
		...
		JOptionPane.showMessageDialog(null,"Calculations Completed Successfully","Calculations",JOptionPane.INFORMATION_MESSAGE);
			...
			if(i==1)
			{
                        try
                            {
				stm = cn.createStatement();
                               stm.executeUpdate ("update Construction set Custname='" +tname.getText()+ "',Itemwork='"+ch.getSelectedItem().toString()+"' ,No=" +ttno.getText()+" ,Lenght=" +ttlen.getText()+" ,Breadth=" +ttbred.getText()+" ,Depth=" +ttdepth.getText()+",Quantity=" +ttquan.getText()+",Totamount="+ttamt.getText()+ " where Custno='"+tcno.getText()+"'");//where Custno='"+tcno.getText()+"'");
                               stm.close();
			...
			t3.setText("");
                ...
                if(e.getSource() == bexit)
                    {
		JOptionPane.showMessageDialog(null,"You Exit  Successfully","Exit",JOptionPane.INFORMATION_MESSAGE);
