...
import java.sql.*;
import java.awt.*;
public class TileScreen extends JFrame implements ActionListener
    {
	...
	Dimension screen 	= 	Toolkit.getDefaultToolkit().getScreenSize();
		...
		setLocation((screen.width - 620)/2,((screen.height-638)/2)-17);	
                ...
                add(ch);
                ch.addItem("Glazed Flooring");
                ch.addItem("Plain Flooring");
                ...
                try
                    {
                       ...
                       /* rs = stm.executeQuery("select * from Tile");
                        rs.next() ;
                        display();*/
                    }
...
try
{
rs=stm.executeQuery("select * from Tile where Custno=(select max(Custno) from Tile)");
...
{}
		...
		tno.setEnabled(true);
			...
			if(i==1)
			{
                        try
                            {
                                sql = "insert into Tile values("+tcno.getText()+",'" +tname.getText()+"','" +ch.getSelectedItem().toString()+"'," +ttno.getText()+"," +ttlen.getText()+"," +ttbred.getText()+","+ttdepth.getText()+","+ttrate.getText()+","+ttquan.getText()+","+ttamt.getText()+")";
                                prstm = cn.prepareStatement(sql);
                                prstm.execute() ;
                                
                                prstm.close();

				String	sql1 = "Update final set TAmt="+ttamt.getText()+" where Name='"+tname.getText()+"'";
			...
			if(i==1)
			{
                        try
                            {
                                stm = cn.createStatement();
				              //sql = "insert into Tile values('" +tname.getText()+"','" +ch.getSelectedItem().toString()+"'," +ttno.getText()+"," +ttlen.getText()+"," +ttbred.getText()+","+ttdepth.getText()+","+ttrate.getText()+","+ttquan.getText()+","+ttamt.getText()+","+tcno.getText()+")";
                               stm.executeUpdate ("update Tile set Custname='" +tname.getText()+ "',Itemwork='"+ch.getSelectedItem().toString()+"' ,No=" +ttno.getText()+" ,Lenght=" +ttlen.getText()+" ,Breadth=" +ttbred.getText()+" ,Depth=" +ttdepth.getText()+",Quantity=" +ttquan.getText()+",Totamount="+ttamt.getText()+ " where Custno="+tcno.getText());//where Custno='"+tcno.getText()+"'");
                               stm.close();

			JOptionPane.showMessageDialog(null,"Record Upddated Successfully","Record Update",JOptionPane.INFORMATION_MESSAGE);
